 ...
 *
 * FIXED:
 *  - __dirname replaced with import.meta.url (ESM compatible)
 *  - startListening() now filters to Saved Messages only (peer "me")
 *    to avoid ingesting videos from other chats
 *  - Hardcoded phone header removed from download path
 */
...
class TelegramClientManager {
   ...
   * Supports HTTP Range requests (RFC 7233) for pause/resume and parallel
   * chunked downloads from the mobile client.
   */
  async streamFileTo(
    messageId: number,
    ...
    const filename = (filenameAttr?.fileName as string | undefined) ?? `video_${messageId}.mkv`;
    ...
    // Align to MTProto requestSize boundary (1 MB)
   ...
   * FIXED: filter to peer "me" only — don't ingest from other chats.
   */
  startListening(): void {
